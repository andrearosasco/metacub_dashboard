...
class URDFLogger:
    ...
    def __init__(self, urdf, rec, root_path: str = "", pos=[0, 0, 0], rpy=[0.0, 0.0, 0.0]) -> None:
        self.urdf = urdf
        self.mat_name_to_mat = {mat.name: mat for mat in self.urdf.materials}
        self.entity_to_transform = {}
        self.root_path = root_path
        self.filepath = urdf.path
        self.rec = rec
    ...
    def log_joint(self, entity_path: str, joint: urdf_parser.Joint) -> None:
        ...
        self.rec.log(self.root_path + entity_path, rr.Transform3D(translation=translation, mat3x3=rotation, axis_length=0.01))
    ...
    def log(self, joint_name: str, value: float) -> None:
        ...
        vec = np.array(np.array(np.round(joint.axis, 6)) * np.round(value, 0) * np.pi / 180.0)

        rot = Rotation.from_rotvec(vec).as_matrix()
        rotation_mat = start_rotation_mat @ rot

        self.rec.log(
            entity_path, rr.Transform3D(translation=start_translation, mat3x3=rotation_mat, axis_length=0.01)
        )
