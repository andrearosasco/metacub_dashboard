...
class DataLogger:
    def __init__(self, path, flush_every=200, exist_ok=False):
        ...
        if Path(path).exists() and exist_ok:
            ...
            self.dest_store = zarr.storage.DirectoryStore(path)
    ...
    def end_episode(self, success=True):
        ...
        try:
            if Path(self.path).exists():
                dest_store = zarr.storage.DirectoryStore(self.path)  # append mode
            else:
                dest_store = zarr.storage.DirectoryStore(self.path)  # write mode
            ...
            if Path(self.path).exists():
                os.remove(self.path)
            self.dest_store = zarr.storage.DirectoryStore(self.path)
            self.dest_group = zarr.group(store=self.dest_store)
...
def write_data(data, path):
    ...
    try:
        if Path(path).exists():
            dest_store = zarr.storage.DirectoryStore(path)  # append mode
        else:
            dest_store = zarr.storage.DirectoryStore(path)  # write mode
        dest_group = zarr.group(store=dest_store, overwrite=False)
    except (zipfile.BadZipFile, OSError, ValueError) as e:
        # If the file is corrupted or has issues, remove it and create a new one
        if Path(path).exists():
            os.remove(path)
        dest_store = zarr.storage.DirectoryStore(path)
